...
Result EquilibriumFluid::calcResult(Params &params) noexcept(false)
{
    calcFunctionValues(params);
    return {std::vector<Vector2>(getPoints(lastCorrectResult)), params.alpha, params.bond, params.relaxationCoef, iterationsCounter};
}

std::vector<Result>* EquilibriumFluid::calcRelaxation(RelaxationParams &params) noexcept(false)
{
    return calcRelaxation(params, nullptr);
}

std::vector<Result>* EquilibriumFluid::calcRelaxation(RelaxationParams &params, std::vector<Vector2> *heightCoefs) noexcept(false)
{
    ...
    do
    {
        try
        {
            ...
            const std::vector<Vector2> &points = getPoints(lastCorrectResult);
        ...
        catch (IncorrectResultException &e)
        {
            problemParams.relaxationCoef /= 2.0;
        }
...
std::vector<Vector2> EquilibriumFluid::getPoints(std::vector<double> &func)
{
...
bool EquilibriumFluid::isCorrect(std::vector<double> &result)
{
...
void EquilibriumFluid::calcFunctionValues(Params &params) noexcept(false)
{
        ...
        lastParams.epsilon != params.epsilon)
    {
        ...
        lastCorrectResult.resize(params.splitNum + 1);
    }
    else
    {
        nextApprox = lastCorrectResult;
    }
    ...
    {
        throw IncorrectResultException();
    }

    lastParams = params;
    lastCorrectResult = nextApprox;
}
